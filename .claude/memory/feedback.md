...
## F13：机器相关的部署/环境行为，外置到本机脚本 + 指针变量，不进仓库或 skill

**规则**：凡是"因机器而异"的执行细节（部署路径、编排文件、宿主机专属命令），不要硬编码进 AInews 仓库文件或 `SKILL.md` 里。改用两层解耦：① 实际执行脚本放在本机的机器级目录（如 `/Volumes/Docker/compose/<project>/`），不进 git；② 仓库根放一个不进 git 的 `.env`，只存一行"脚本在哪"的指针变量，`skill` 侧只管读指针、调用脚本、看退出码，不关心脚本内部做了什么。

**Why**：
- 2026-07-02 F2.7 部署编排两轮重构：第一轮先把 Docker builder 判断为过度设计（build 该留仓库自己做，因为天然依赖仓库内容；只有"发布"是机器相关的）；第二轮用户进一步要求把 rsync/compose up/冒烟测试这段也从 SKILL.md 里抽出去
- 用户明确点出这是"抽象接口设计哲学"：不同机器的部署行为可以自定义，"完全外接到本地脚本上"才是合理的解耦方式——仓库/skill 只依赖一个稳定接口（"调用指针指向的脚本，看退出码"），不依赖某台机器的具体路径
- 好处是可移植：换机器只需要重写一份 `.env`，不用碰仓库任何文件；也符合 AInews 一直以来"vault ↔ 管道解耦"的同一条原则的自然延伸（这次是"管道 ↔ 部署基础设施解耦"）

**How to apply**：
- 遇到"这个命令/路径只在这台机器上有意义"的东西（Docker 编排、本机数据目录、宿主机专属 cron、机器绑定的密钥路径）→ 第一反应是"这应该是本机配置，不是仓库逻辑"
- 落地模式：仓库根或项目根放 `.env`（`.gitignore` 排除）+ 提交一份 `.env.example` 占位（保留机制可发现性）；真正的脚本放在本机的机器级统一目录（本机是 `/Volumes/Docker/compose/<project>/`），不与仓库同层
- skill/主流程侧只做三件事：读指针变量 → 校验指向的文件存在且可执行（不存在就跳过，不是报错）→ 调用并看退出码；不要在 skill 里重复实现脚本内部的具体命令
- 这条原则不止适用于 Docker/部署，任何"跨仓库边界的机器专属行为"都该用同一个模式
