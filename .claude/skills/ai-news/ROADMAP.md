...
最后更新：2026-07-01 · v2.4 后 F1/F2 规划确认 · **F2.0 框架 POC 完成**
...
- **F2.0** 框架 POC（2026-07-01，commit tbd）：**采用 Quartz 5**——Quartz / Astro / Hugo 三选一实测，Quartz 5 得 38/40（Astro 30 / Hugo 26），独家原生 Obsidian Bases 支持 + 46 插件内置（wikilink/backlinks/graph/search/RSS/OG/dark mode/reader/TOC 全内置），净省 F2.1-F2.7 约 2 天。报告见 [`.claude/skills/ai-news/notes/F2.0-poc-report.md`](../../.claude/skills/ai-news/notes/F2.0-poc-report.md)。副产品：news-originalizer.md YAML frontmatter bug（`original_title` 含冒号未加引号）待补约束。
...
| # | 任务 | 成本 | 输出 |
|---|---|---|---|
| **F2.0** | ✅ 框架 POC 3 选 1（Quartz 5 / Astro / Hugo）—— **决策：Quartz 5**，见 `notes/F2.0-poc-report.md` | ~2h（实际，估 1.5day） | 决策文档 + `web/poc-quartz/` 骨架 |
