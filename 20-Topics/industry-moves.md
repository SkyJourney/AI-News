---
created: 2026-06-28 12:30:00
updated: 2026-07-01 09:31:00
entry_total: 15
---

# 产业动态 (industry-moves)

追踪公司层面的战略动作：合作、收购、扩张、关停、人才流动及战略转向。

## 2026-07-01

- **乐动机器人港股押注物理 AI，打造万亿市场核心基础设施**（未升级 Zettel）
  港股新贵乐动机器人战略定位物理 AI 基础设施，新一轮资本向具身智能赛道聚拢。
  源：`qbitai`
## 2026-06-29

- **HP Inc. launches Frontier strategic partnership with OpenAI**（未升级 Zettel）
  HP 将 OpenAI Frontier 合作伙伴关系规模化延伸至客户体验、软件开发与企业运营三条业务线。
  源：`openai-rss`
- **Patch the Planet: a Daybreak initiative**（未升级 Zettel）
  OpenAI Daybreak 计划新增开源安全方向，AI 辅助开源维护者发现并修复漏洞。
  源：`openai-rss`
- **Anthropic opens Seoul office**（未升级 Zettel）
  Anthropic 首尔办公室落地，宣布与韩国 AI 生态多家机构达成合作——国际化扩张提速。
  源：`anthropic-news`
- **State of AI Report 2025** ([[202606291405-state-of-ai-report-2025]])
  第八届年度报告发布，覆盖研究/产业/政策/安全四大维度，配套 1,200+ 从业者调查。
  源：`state-of-ai`
- **State of AI Report 2025 - Full Slide Deck**（未升级 Zettel）
  完整幻灯片版辅助参考，与主报告内容一致。
  源：`state-of-ai`
- **AI Practitioner Survey 2025**（未升级 Zettel）
  首届大规模 AI 从业者调查，1,200+ 参与者。
  源：`state-of-ai`
- **State of AI Report 2025 Launch**（未升级 Zettel）
  报告发布博客，与主报告内容重复。
  源：`state-of-ai`

## 2026-06-28

- **Introducing Claude Tag**（未升级 Zettel）
  Anthropic 发布 Claude Tag，为团队提供新的协作与内容追踪能力。
  源：`anthropic-news`

- **Anthropic 首尔办公室开业 + 韩国 AI 生态合作**（未升级 Zettel，低置信度）
  Anthropic 宣布首尔新办公室并与韩国 AI 生态伙伴签署战略合作，加速亚太地区布局。
  源：`anthropic-news`

- **TCS × Anthropic：将 Claude 引入受监管行业**（未升级 Zettel，低置信度）
  塔塔咨询服务与 Anthropic 战略合作，聚焦金融、医疗等受监管行业的 Claude 部署。
  源：`anthropic-news`

- **DXC × Anthropic：将 Claude 嵌入银行/航空等关键基础设施**（未升级 Zettel，低置信度）
  DXC Technology 与 Anthropic 结盟，将 Claude 集成至银行、航空等关键行业系统。
  源：`anthropic-news`

- **Samsung Electronics 向全球员工部署 ChatGPT 和 Codex**（未升级 Zettel，低置信度）
  三星全球规模员工 AI 工具铺开，是 OpenAI 迄今最大的企业 AI 部署案例之一。
  源：`openai-rss`

- **Patch the Planet: Daybreak 开源维护者扶持计划**（未升级 Zettel）
  OpenAI Daybreak 旗下 Patch the Planet 帮助开源维护者用 AI 批量发现并修复安全漏洞。
  源：`openai-rss`

- **谷歌"推理之王"研究员跳槽 Meta**（未升级 Zettel，低置信度）
  Google DeepMind 头部推理研究员加入 Meta AI，折射两家在前沿推理领域的人才争夺。
  源：`qbitai`

